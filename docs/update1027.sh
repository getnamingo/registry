...
echo "This will update Namingo Registry from v1.0.26 to v1.0.27."
...
echo "Cloning v1.0.27 from the repository..."
git clone --branch v1.0.27 --single-branch https://github.com/getnamingo/registry /opt/registry1027
...
copy_files "/opt/registry1027/automation" "/opt/registry/automation"
copy_files "/opt/registry1027/cp" "/var/www/cp"
copy_files "/opt/registry1027/whois/web" "/var/www/whois"
copy_files "/opt/registry1027/das" "/opt/registry/das"
copy_files "/opt/registry1027/whois/port43" "/opt/registry/whois/port43"
copy_files "/opt/registry1027/rdap" "/opt/registry/rdap"
copy_files "/opt/registry1027/epp" "/opt/registry/epp"
copy_files "/opt/registry1027/docs" "/opt/registry/docs"
...
# Remove unnecessary UDP rules from older installs
ufw delete allow 22/udp 2>/dev/null || true
ufw delete allow 80/udp 2>/dev/null || true
ufw delete allow 700/udp 2>/dev/null || true
ufw delete allow 1043/udp 2>/dev/null || true

# Ensure correct rules exist
ufw allow 22/tcp
ufw allow 43/tcp
ufw allow 80/tcp
ufw allow 443/tcp
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registry1027..."
    rm -rf /opt/registry1027
else
    echo "There was an issue starting the services. /opt/registry1027 will not be deleted."
fi

echo "Upgrade to v1.0.27 completed successfully."