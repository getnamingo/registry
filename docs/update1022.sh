...
echo "This will update Namingo Registry from v1.0.21 to v1.0.22."
...
echo "Cloning v1.0.22 from the repository..."
git clone --branch v1.0.22 --single-branch https://github.com/getnamingo/registry /opt/registry1022
...
copy_files "/opt/registry1022/automation" "/opt/registry/automation"
copy_files "/opt/registry1022/cp" "/var/www/cp"
copy_files "/opt/registry1022/whois/web" "/var/www/whois"
copy_files "/opt/registry1022/das" "/opt/registry/das"
copy_files "/opt/registry1022/whois/port43" "/opt/registry/whois/port43"
copy_files "/opt/registry1022/rdap" "/opt/registry/rdap"
copy_files "/opt/registry1022/epp" "/opt/registry/epp"
copy_files "/opt/registry1022/docs" "/opt/registry/docs"
...
echo "Creating registrar_price_group table..."
...
# Update secdns.digest length from 64 to 96
echo "Updating secdns.digest column length to 96..."
mysql -u$DB_USER -p$DB_PASS $DB_NAME -e "ALTER TABLE secdns MODIFY COLUMN digest VARCHAR(96) NOT NULL;"

echo "Database structure updated successfully."

# Patch WHOIS/RDAP config
RDAP_CONFIG="/opt/registry/rdap/config.php"
WHOIS_CONFIG="/opt/registry/whois/port43/config.php"

echo "Patching RDAP config..."
if ! grep -q "limited_rdap" "$RDAP_CONFIG"; then
    sed -i "/'minimum_data' *=> */a\    'limited_rdap' => false," "$RDAP_CONFIG"
fi

echo "Patching WHOIS config..."
if ! grep -q "limited_whois" "$WHOIS_CONFIG"; then
    sed -i "/'minimum_data' *=> */a\    'limited_whois' => false," "$WHOIS_CONFIG"
fi
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registry1022..."
    rm -rf /opt/registry1022
else
    echo "There was an issue starting the services. /opt/registry1022 will not be deleted."
fi

echo "Upgrade to v1.0.22 completed successfully."