...
echo "This will update Namingo Registry from v1.0.30 to v1.0.31."
...
case "$db_driver" in
    mysql|mariadb)
        for backup_db in "${databases[@]}"; do
            echo "Backing up MariaDB/MySQL database $backup_db..."
            ...
            mariadb-dump \
                ...
                -p"$db_pass" \
                -h"$db_host" \
                "$backup_db" > "$sql_backup_file"

            echo "Compressing database backup $backup_db..."
            tar -czf "${sql_backup_file}.tar.gz" \
                -C "$backup_dir" \
                "$(basename "$sql_backup_file")"
    ...
    pgsql|postgres|postgresql)
        for backup_db in "${databases[@]}"; do
            echo "Backing up PostgreSQL database $backup_db..."
            sql_backup_file="$backup_dir/db_${backup_db}_backup_$(date +%F).sql"

            PGPASSWORD="$db_pass" pg_dump \
                -h "$db_host" \
                -U "$db_user" \
                -d "$backup_db" > "$sql_backup_file"
...
echo "Cloning v1.0.31 from the repository..."
git clone \
    --branch v1.0.31 \
    --single-branch \
    --depth 1 \
...
copy_files "/opt/registry1031/automation" "/opt/registry/automation"
copy_files "/opt/registry1031/cp" "/var/www/cp"
copy_files "/opt/registry1031/whois/web" "/var/www/whois"
[ -d "/opt/registry/das" ] && copy_files "/opt/registry1031/das" "/opt/registry/das"
[ -d "/opt/registry/whois/port43" ] && copy_files "/opt/registry1031/whois/port43" "/opt/registry/whois/port43"
copy_files "/opt/registry1031/rdap" "/opt/registry/rdap"
copy_files "/opt/registry1031/epp" "/opt/registry/epp"
copy_files "/opt/registry1031/docs" "/opt/registry/docs"
...
if [[ $? -eq 0 ]]; then
    echo "Services started successfully. Deleting /opt/registry1031..."
    rm -rf -- "$clone_dir"
else
    echo "There was an issue starting the services. /opt/registry1031 will not be deleted."
fi

echo "Upgrade to v1.0.31 completed successfully."