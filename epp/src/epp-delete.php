...
function processContactDelete($conn, $db, $xml, $clid, $database_type, $trans) {
    ...
    try {
        ...
        sendEppError($conn, $db, 2400, 'Contact was not deleted due to a database error', $clTRID, $trans);
        return;
    }
...
function processHostDelete($conn, $db, $xml, $clid, $database_type, $trans) {
    ...
    $query = "SELECT status FROM host_status WHERE host_id = :host_id";
    $stmt = $db->prepare($query);
    $stmt->execute([':host_id' => $host_id]);
    $statuses = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $stmt->closeCursor();
    ...
    foreach ($statuses as $status) {
        if (preg_match('/(UpdateProhibited|DeleteProhibited)$/', $status) || preg_match('/^pending/', $status)) {
            sendEppError($conn, $db, 2304, 'The host has a status that does not allow deletion', $clTRID, $trans);
            return;
        }
    }
    ...
    try {
        ...
        $stmt = $db->prepare("DELETE FROM host_status WHERE host_id = :host_id");
        $stmt->execute([':host_id' => $host_id]);
        ...
        sendEppError($conn, $db, 2400, 'The host was not deleted due to a database error', $clTRID, $trans);
        return;
    }
...
function processDomainDelete($conn, $db, $xml, $clid, $database_type, $trans) {
    ...
    if (isset($launch_delete)) {
        ...
        try {
            ...
            $stmt = $db->prepare("DELETE FROM application_contact_map WHERE domain_id = ?");
            $stmt->execute([$domain_id]);

            $stmt = $db->prepare("DELETE FROM application_host_map WHERE domain_id = ?");
            $stmt->execute([$domain_id]);

            $stmt = $db->prepare("DELETE FROM application_status WHERE domain_id = ?");
            $stmt->execute([$domain_id]);
            ...
            sendEppError($conn, $db, 2400, 'The application has not been deleted due to a database error', $clTRID, $trans);
            return;
        }
        ...
        try {
            ...
            sendEppError($conn, $db, 2400, 'The domain name has not been deleted due to a database error', $clTRID, $trans);
            return;
        }
    ...
    $epp = new EPP\EppWriter();
    $xml = $epp->epp_writer($response);
    updateTransaction($db, 'delete', 'domain', $domainName, $response['resultCode'], $response['message'], $svTRID, $xml, $trans);
    sendEppResponse($conn, $xml);
}